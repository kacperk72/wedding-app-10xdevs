...
tech_stack:
  ...
  framework: Angular 20+ standalone (frontend) + Express + Supabase Postgres (backend)
  runtime: Node.js 22 LTS on Hostinger Node.js application slot; Supabase Postgres (managed, off-host) for wedding-planner data; MySQL on the Hostinger plan belongs to the separate SSO app only
...
> **Korekta stacku (2026-06-08):** ten dokument powstał 2026-05-20 przed pivotem
> bazy danych i w kilku miejscach mówi „Express + Sequelize + MySQL" dla
> wedding-planner. **To nieaktualne.** Dane wedding-planner żyją w **Supabase
> Postgres** (klient `@supabase/supabase-js` `service_role`, migracje
> `npx supabase db push`, RLS deny-all). MySQL na planie Hostinger należy
> **wyłącznie do SSO** (`kubitksso.pl`). Decyzje hostingowe poniżej (Hostinger
> Business, FTP dla SPA, Node.js app dla backendu, GitHub Actions) **pozostają
> aktualne** — przestarzałe są tylko wzmianki o MySQL/Sequelize jako bazie
> wedding-planner. Patrz `CLAUDE.md` §„Current stack".

## Recommendation

**Deploy on Hostinger Business** — Angular SPA via FTP to `/public_html/` of `wedding-planner-kubitk.pl`, Express backend as a Hostinger Node.js application on the same plan, with wedding-planner data in **Supabase Postgres** (managed, off-host; the Hostinger MySQL is the SSO app's, not wedding-planner's). CI/CD split by layer: **GitHub Actions** runs the frontend pipeline (FTP-Deploy-Action), **Hostinger Git auto-pull** handles the backend (Hostinger watches `main`, runs `npm install`, restarts on every push). Backend currently lives on the auto-assigned subdomain `https://deeppink-mole-431102.hostingersite.com`; the frontend will live on `https://wedding-planner-kubitk.pl` — cross-origin, so `FRONTEND_ORIGIN` on the backend env is load-bearing.
...
| A Supabase migration run on prod that does `DROP COLUMN` or `ALTER COLUMN` with data loss → unrecoverable except from backup | Pre-mortem-style | M | H | Migration rule: additive only on prod (`ADD COLUMN`, `ADD INDEX`, `CREATE TABLE`) via `npx supabase db push`. Destructive migrations require a manual run in a maintenance window with a pre-run snapshot (Supabase dashboard backup / `pg_dump`). Already documented in CLAUDE.md (§Migrations + drift check). |
...
2. **Create the Node.js application** in Hostinger panel pointing at the backend directory (e.g. `~/wedding-planner-backend/src/server.js`, Node 22). Note the application's environment-variables panel — that is where `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY`, the SSO JWKS URL (`https://kubitksso.pl/.well-known/jwks.json`), and the app slug (`wedding-planner`) live. Empty for now.
3. **Provision the Supabase Postgres project** for wedding-planner (managed, off-host — not a Hostinger MySQL database). Copy `SUPABASE_URL` and the `service_role` key into the Node.js app's environment-variables panel from step 2, then apply the schema with `npx supabase db push` from `wedding-planner/backend/`. _(The Hostinger MySQL database belongs to the separate SSO app; wedding-planner does not create one.)_
